...
void start_hash(Hash* hash, int tamanho);
void freeHash(Hash* hash);
int h(char elemento[], int m);
bool hash_pesquisar(Hash* hash, char elemento[]);
void hash_inserir(Hash* hash, Pokemon elemento);
...
int main () {
    ...
    while(strcmp(line, "FIM") != 0) {
        ...
        hash_inserir(&hash, clone(&pokedex[num-1]));
        scanf(" %[^\n]", line);
    }
    ...
    while(strcmp(entrada, "FIM") != 0) {
        printf("=> %s: ", entrada);
        if(hash_pesquisar(&hash, entrada)) {
            printf("SIM\n");
        } else {
            printf("NAO\n");
        }
...
void start_hash(Hash* hash, int tamanho) {
    ...
    hash->tabela = malloc(tamanho * sizeof(Lista));
    if (hash->tabela == NULL) {
        exit(EXIT_FAILURE);
    }
...
bool hash_pesquisar(Hash* hash, char elemento[]) {
    ...
    bool resp = pesquisar(elemento, &hash->tabela[pos]);
    if(resp) {
...
void hash_inserir(Hash* hash, Pokemon elemento) {
    int pos = h(getName(&elemento), hash->tamanho);
    inserirInicio(elemento, &hash->tabela[pos]);
}
...
void inserir(Pokemon x, int pos, Lista* lista) {
    ...
    //validar insercao
    if(lista->n >= NUM_POKEMONS || pos < 0 || pos > lista->n){
        printf("Erro ao inserir!");
        exit(1);
    }
...
bool pesquisar(char x[], Lista* lista) {
    ...
    for (i = 0; i < lista->n && retorno == false; i++) {
        if(strcmp(getName(&lista->array[i]), x) == 0) {
            retorno = true;
        }
    }
