...
int main () {
    ...
    while(strcmp(line, "FIM") != 0) {
        ...
        inserirAVL(&avl, clone(&pokedex[num-1]));
        scanf(" %[^\n]", line);
    }
...
bool pesquisar(char name[], No* i) {
    ...
    if (i == NULL) {
        resp = false;
    } else if (strcmp(name, getName(&i->pokemon)) == 0) {
        resp = true;
    } else if (strcmp(name, getName(&i->pokemon)) < 0) {
        printf("esq ");
...
No* inserir(No* no, Pokemon p) {
    if (no == NULL) {
        no = novoNo(p);
    } else if (strcmp(getName(&p), getName(&no->pokemon)) < 0) {
